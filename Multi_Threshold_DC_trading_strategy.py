                ...
                if event_dict[dc][-1]: 
                    if self.data['Close'][idx] <= (ph[dc_idx] * (1 - dc)):
                        ...
                        if ph[dc_idx] < self.data['Close'][idx]:
                            ...
                            try:
                                if event_dict[dc][-3] == False and event_dict[dc][-2] == True: #[False], [True], [True] -> the current time point is right after the confirmation point, yet it is not low enough to be directional change, but lower than or same as the confirmation point
                                    tdc_0[dc_idx] = idx - 1 
                            except:
                                pass  
